...
"""K6 turbo_gla_output 单算子测试脚本（本目录自包含）。

你只需读懂/改动的文件是本目录的 `turbo_gla_output_kernel.py`（含 torch 参考 +
...
正确性判定: 输出 o triton-vs-torch max_diff < 1e-2 即 OK（K6 目标 case 约 1e-6 量级）。
...
import turbo_gla_output_kernel as K  # noqa: E402

TARGET = dict(B=1, T=16384, H=96, K=128, V=128)
MAX_DIFF = 1e-2
# 目标 case H%16==0 → HM 头合并版 `chunk_gla_fwd_kernel_o_hm`；小 H 退化到
# `chunk_gla_fwd_kernel_o`。两者同前缀，故用前缀过滤（substring 匹配）。
...
def _build(B, T, H, Kk, V, device, seed):
    ...
    g = (torch.randn(B, T, H, Kk) * 0.5 - 2.0).to(device)   # log2 空间累积 gate（负）
    # Aqk: chunk 内因果注意力权重（行 t、列=chunk 内位置）。真实值只有下三角有意义
    # （torch 参考与 triton 都用 tril 掩码丢弃上三角），这里给全随机以覆盖掩码路径。
    Aqk = (torch.randn(B, T, H, BT) * 0.1).to(device)
    # h: 每个 chunk 起始的压缩状态快照 [B, NT, H, V, K]
...
def _run_torch(i):
    return [K.turbo_gla_output_torch(i["q"], i["v_new"], i["g"], i["Aqk"], i["h"],
                               i["scale"], chunk_size=BT)]


def _run_triton(i):
    return [K.turbo_gla_output_triton(i["q"], i["v_new"], i["g"], i["Aqk"], i["h"],
                                i["scale"], chunk_size=BT)]
...
# 边界 shape：覆盖尾 chunk 不满 / K=V=32 / 128 / V≠K / 多 head(H=8) / 多 chunk / 多 batch
SELFTEST = [
...
def _perf(repeats, warmup):
    ...
    print("  msprof 包裹: msprof --output=./prof_k6 --application=\""
...
def main(argv=None):
    p = argparse.ArgumentParser(description="K6 turbo_gla_output 测试/复现脚本")
    ...
    print(f"[{OP}] gla_output 目标 case B{TARGET['B']} T{TARGET['T']} H{TARGET['H']} "

...
"""K2 turbo_token_parallel 单算子测试脚本（本目录自包含）。

你只需读懂/改动的文件是本目录的 `turbo_token_parallel_kernel.py`（含 torch 参考 +
...
正确性判定: 每个输出 triton-vs-torch max_diff < 1e-2 即 OK（K2 目标 case 基线约 1e-7）。
性能口径: msprof `Task Duration(us)` 每调用均值。目标 case 官方基线与门槛见 ../TEST_REPORT.md（§4）；优化目标是让这个数字下降且精度不劣于 1e-2。
"""
...
import turbo_token_parallel_kernel as K  # noqa: E402
...
def _run_torch(i):
    return K.turbo_token_parallel_torch(i["q"], i["k"], i["gk"], i["beta"], i["scale"],
                                  chunk_size=BT, sub_chunk_size=BC)


def _run_triton(i):
    return K.turbo_token_parallel_triton(i["q"], i["k"], i["gk"], i["beta"], i["scale"],
                                   chunk_size=BT, sub_chunk_size=BC)
...
# 边界 shape：覆盖尾 chunk 不满 / K=128 / K=32 / 多 head(H=8) / 多 chunk
SELFTEST = [
    ("tiny_k64", 1, 128, 2, 64),
    ("tail_t63", 1,  63, 2, 64),
...
def _perf(repeats, warmup):
    ...
    print("  msprof 包裹: msprof --output=./prof_k2 --application=\""
...
def main(argv=None):
    p = argparse.ArgumentParser(description="K2 turbo_token_parallel 测试/复现脚本")
    ...
    print(f"[{OP}] token_parallel 目标 case B{TARGET['B']} T{TARGET['T']} H{TARGET['H']} "

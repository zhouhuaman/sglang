...
bf16 改造(2026-09-09,基于 fp16 版转制;对齐 Ascend C bf16 口径):输入 q/k/gk/beta 与输出 Aqk/Akk(含内部
...
def _token_parallel_kernel(
    ...
    Aqk_full = tl.dot(qe.to(tl.bfloat16), tl.trans(ke.to(tl.bfloat16)))  # [BT, BT]
    kbe = (kc * betac[:, None]) * tl.math.exp2(gc)                     # fp32
    Akk_full = tl.dot(kbe.to(tl.bfloat16), tl.trans(ke.to(tl.bfloat16)))  # [BT, BT]
    ...
    r_o = chunk_start + o_r
    tl.store(base_aqk + r_o[:, None] * H * BT + tl.arange(0, BT)[None, :],
             Aqk_full.to(Aqk.dtype.element_ty))
    tl.store(base_akk + r_o[:, None] * H * BT + tl.arange(0, BT)[None, :],
             Akk_full.to(AkkScratch.dtype.element_ty))
...
def _token_parallel_kernel_hm2(
    ...
    for hh in range(HM):
        ...
        Aqk_full = tl.dot(qe.to(tl.bfloat16), tl.trans(ke.to(tl.bfloat16)))  # acc fp32
        kbe = (kc * betac[:, None]) * eg   # fp32 vector 段
        Akk_full = tl.dot(kbe.to(tl.bfloat16), tl.trans(ke.to(tl.bfloat16)))  # acc fp32
...
def turbo_token_parallel_triton(
    ...
    TP = NT * BT
    dev = q.device
    # fp16 形态(2026-09-09): 输入统一转 fp16 再进 kernel (已是 fp16 则 .to 为 no-op)
    ...
    # fp16: 输出 Aqk 及内部 scratch 缓冲统一 fp16
    if Aqk is None:
        Aqk = torch.empty(B, TP, H, BT, device=dev, dtype=torch.bfloat16)
    ...
    if K == BK:
        ...
        grid = (NT, B * (H // HM))
        scratch = torch.empty(B, TP, H, BT, device=dev, dtype=torch.bfloat16)
        if Akk is None:
            Akk = torch.empty(B, TP, H, BC, device=dev, dtype=torch.bfloat16)

...
"""K5 turbo_delta_rule_h 单算子测试脚本（本目录自包含）。

你只需读懂/改动的文件是本目录的 `turbo_delta_rule_h_kernel.py`（含 torch 参考 +
...
import turbo_delta_rule_h_kernel as K  # noqa: E402
...
def _run_torch(i):
    ...
    return K.turbo_delta_rule_h_torch(
        i["k"], i["w"], i["u"], i["gk"], i["initial_state"].clone(),
        i["indices"], chunk_size=BT)


def _run_triton(i):
    return K.turbo_delta_rule_h_triton(
        i["k"], i["w"], i["u"], i["gk"], i["initial_state"].clone(),
        i["indices"], chunk_size=BT)
...
def main(argv=None):
    p = argparse.ArgumentParser(description="K5 turbo_delta_rule_h 测试/复现脚本")

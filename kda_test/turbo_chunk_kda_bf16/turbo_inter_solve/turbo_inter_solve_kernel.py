...
def turbo_inter_solve_triton(
    ...
    TP = NT * BT
    dev = q.device
    # bf16 形态(B): 输入统一 bf16、缓冲 bf16, kernel 内 fp32 cube(见 EXPERIMENT_BF16.md:
    # bf16 dot 与 fp16 dot 一样触发 3.2.1 深依赖链 507015, 维持 B 形态)
    ...
    if Akk_out is None:
        Akk_out = torch.empty(B, TP, H, BT, device=dev, dtype=torch.bfloat16)

...
def chunk_gla_fwd_kernel_o(
    ...
    """Route B: manual pointer arithmetic replacing tl.make_block_ptr.

    grid = (cdiv(V, BV), NT, B * H); each program handles one
    ``(V-tile, chunk, (batch, head))`` intersection and writes a ``[BT, BV]``
    sub-block:
      * all tensor accesses are computed by hand as base + offset, no block_ptr;
      * explicit row/col masks for boundary checks;
      * fp32 causal mask;
      * mathematically fully equivalent to the original implementation.

    Second-round optimization: the BK of the cross-chunk K loop is controlled by the
    driver (BK=min(K,128), num_warps=2). For the target case K=128, going from BK 32 to
    128 merges the 4 serial small dots into one [64,128]@[128,128], 8.2ms -> 6.8ms
    (isolated experiments confirmed the serial K loop is the bottleneck).
    """
    i_v = tl.program_id(0)   # V-dim tile index
    ...
    NT = tl.cdiv(T, BT)
    i_tg = i_b * NT + i_t    # global tile index (used to access h)
    bos = i_b * T

    # compile-time strides
    s_q_t: tl.constexpr = H * K       # q/g stride along the T dim
    s_v_t: tl.constexpr = H * V       # v/o stride along the T dim
    s_h_v: tl.constexpr = K           # h stride along the V dim
    s_a_t: tl.constexpr = H * BT      # A stride along the T dim

    # lower-triangular causal mask: m_s[i, j] = (i >= j)  (fp32)
    m_s = tl.arange(0, BT)[:, None].to(tl.float32) >= tl.arange(0, BT)[None, :].to(tl.float32)

    b_o = tl.zeros([BT, BV], dtype=tl.float32)

    # ── cross-chunk path: sequential loop over K (manual pointer arithmetic) ──
    ...
    # ── intra-chunk path: Aqk @ v_new (causal) ──
...
def chunk_gla_fwd_kernel_o_hm(
    ...
    """Head-merged version (HM heads/CTA, grid=(cdiv(V,BV), NT, B*H//HM)).

    **Scalar-addressing reduction (2026-08-25, fifth round, see OPTIMIZATION_LOG.md)**:
    msprof showed the original kernel at 97.8% aiv_scalar (vector scalar/address
    generation saturated). The per-CTA fixed scalar setup (arange vectors, causal mask
    m_s, the various boundary masks, offset vectors) depends only on (i_v, i_t, chunk),
    not on head -- the HM head-merge loop amortizes that fixed setup HM times, and every
    loop-invariant vector is hoisted above the head loop. Target case 6.94ms -> 4.75ms
    (-32%), fully bit-exact (max_diff 0.0 vs the original kernel).

    The math is identical to ``chunk_gla_fwd_kernel_o`` (cross-chunk q_gated@h^T +
    intra-chunk A_masked@v_new); each head accumulates its own b_o and writes it back
    independently.
    """
    i_v = tl.program_id(0)   # V-dim tile index
    i_t = tl.program_id(1)   # chunk index
    i_hg = tl.program_id(2)  # (batch, head-group) index
    NT = tl.cdiv(T, BT)
    n_hg = H // HM
    i_b = i_hg // n_hg
    hg0 = i_hg % n_hg
    ...
    # ── loop-invariant vectors (independent of head; all hoisted out of the head loop) ──
    r = tl.arange(0, BT)
    c = tl.arange(0, BT)
    m_s = r[:, None].to(tl.float32) >= c[None, :].to(tl.float32)   # causal mask [BT,BT]
    r_mask = (i_t * BT + r) < T

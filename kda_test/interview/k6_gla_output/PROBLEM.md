...
官方基线 ≈ **3.91 ms/调用**（±10%，2026-09-07 本机 triton-ascend 3.2.1 msprof 实测口径）；
门槛 `max_diff < 1e-2`。
...
- 【双路单累加器】跨块 `o_cross = qg @ h_cᵀ` 与块内 `o_intra = tril(A_c) @ v_new_c`
  累进**同一个 fp32 寄存器累加器**：无中间张量落 HBM，整块输出一次写回 —— 两路的
  权重结构（各自乘什么）保留，只是共享了累加器。
- 【因果内建 → 掩码减半】`h_c` 是 chunk c **起始**的快照、只含本 chunk 之前的信息 ⇒
  跨块一路天然因果、**零掩码**；只有块内一路需要 `tl.where(r>=c, A, 0)` 下三角。
  若把 h 换成"到当前为止"的 state 就得两路都掩 —— 结构选择直接砍掉一半掩码工作。
- 【head 无关量的循环外提升】与 head 无关的因果掩码/边界掩码/偏移向量全部提到
  `for hh in range(HM)` 之外：目标 case 6.94→4.75ms；且瓶颈随之**转移**
  （aiv_scalar 97.8%→60.9%，新瓶颈 aic_mte2 92.5%）—— 这证明 per-CTA 标量 setup
  是真实隐藏成本：摊薄 16× 之后才轮到访存说话。
- 【K 维循环合并】跨块部分 BK 32→128 合并成单条大 dot（8.2→6.8ms）：目标 K=128 时
  跨块/块内各恰好一条 dot —— dot 粒度每放大一倍，围绕它的标量脚手架就少一份。
- 【负结果与饱和点扫描】HM=32 vs HM=16（4.74 vs 4.75ms）说明摊薄收益饱和；用
  block_ptr 全重写 7.0ms **证伪**了"手动 int64 寻址耗标量"的假设（耗标量的是每
  CTA setup，不是寻址形态）；fp16 dot 被后端 upcast 回 fp32（cube 未启用）→ 保持
  fp32 路径。负结果一样入库，防止后人重复实验。
- 【瓶颈判定方法示范】先用 ai-core 隔离 profile **证伪"内存下界"假设**（R4 测得
  aiv_scalar 97.8% 顶满，而非 mte2），再定位到标量地址生成 —— "先证伪再动手"的
  诊断流程是这类算子优化的通用方法论。

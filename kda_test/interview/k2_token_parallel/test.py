...
"""K2 token_parallel 面试题测试脚本（本目录自包含）。

你只需读懂/改动的文件是本目录的 `token_parallel_kernel.py`（含 torch 参考 +
...
9.5–9.8ms/调用（±10%）；优化目标是让这个数字下降且精度不劣于 1e-2。
"""
...
def _report(dirpath):
    ...
    print("\n  目标 case 官方隔离基线 ≈9.5–9.8ms/调用（±10%）。"
          "优化目标是下降这个均值且精度仍 <1e-2。")
    return 0


def main(argv=None):
    p = argparse.ArgumentParser(description="K2 token_parallel 测试/复现脚本")

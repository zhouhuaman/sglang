...
"""K4 recompute_w_u 面试题测试脚本（本目录自包含）。

你只需读懂/改动的文件是本目录的 `recompute_w_u_kernel.py`（含 torch 参考 +
...
import recompute_w_u_kernel as K  # noqa: E402
...
def _run_torch(i):
    return K.recompute_w_u_torch(i["k"], i["v"], i["beta"], i["A"], i["gk"],
                                 chunk_size=BT)


def _run_triton(i):
    return K.recompute_w_u_triton(i["k"], i["v"], i["beta"], i["A"], i["gk"],
                                  chunk_size=BT)
...
def main(argv=None):
    p = argparse.ArgumentParser(description="K4 recompute_w_u 测试/复现脚本")

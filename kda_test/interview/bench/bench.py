...
"""KDA 6 算子统一 bench（interview/bench 版）: 正确性（模式 A）+ msprof 分段采集（模式 B）。

本目录 kernel 从 ../k1_gate_chunk_cumsum/ ~ ../k6_gla_output/ 的独立实现导入
（与面试包各题 *_kernel.py 同源），替代 design/<op>/src 版，其余流程不变。
...
# 把 interview 的 6 个题目目录加进 sys.path，以便 import 各 kernel 模块。
# 注意 interview 与 design/<op>/src 的模块/函数命名不同:
#   K1: gate_chunk_cumsum_kernel.py（gate_chunk_cumsum_triton / _torch / ref）
#   K6: driver 名为 gla_output_triton（design 里叫 gla_output_kernel）
for d in ("k1_gate_chunk_cumsum", "k2_token_parallel", "k3_inter_solve",
          "k4_recompute_w_u", "k5_delta_rule_h", "k6_gla_output"):
...
# 6 个算子的 driver 函数
from gate_chunk_cumsum_kernel import (  # noqa: E402
    RCP_LN2,
    gate_chunk_cumsum_triton as k1_triton,
    gate_chunk_cumsum_torch as k1_torch,
)
...
from gla_output_kernel import (  # noqa: E402
    ...
    gla_output_triton as k6_triton,
)

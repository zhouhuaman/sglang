...
"""K1 turbo_gate_chunk_cumsum 单算子测试脚本（本目录自包含）。

你只需读懂/改动的文件是本目录的 `turbo_gate_chunk_cumsum_kernel.py`（含 torch 参考 +
...
import turbo_gate_chunk_cumsum_kernel as K  # noqa: E402
...
def _run_torch(i):
    return [K.turbo_gate_chunk_cumsum_torch(i["x"], i["A_log"], i["dt_bias"],
                                      chunk_size=BT)]


def _run_triton(i):
    return [K.turbo_gate_chunk_cumsum_triton(i["x"], i["A_log"], i["dt_bias"],
                                       chunk_size=BT)]
...
def main(argv=None):
    p = argparse.ArgumentParser(description="K1 turbo_gate_chunk_cumsum 测试/复现脚本")

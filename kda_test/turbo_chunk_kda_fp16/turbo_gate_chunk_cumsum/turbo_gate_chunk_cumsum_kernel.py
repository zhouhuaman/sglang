...
fp16 改造(2026-09-09,对齐 Ascend C 口径):激活张量 x 输入 fp16,vector 段 fp32,
无 dot;A_log/dt_bias 参数与输出 gk 保持 fp32(目标 case gk 量级 O(1e3),fp16 存储
误差 ~0.2 超 1e-2 门槛,实测 0.187 FAIL → 修正为参数/输出 fp32)。
...
def turbo_gate_chunk_cumsum_triton(
    ...
    返回:
        [B, T, H, K] fp16 结果（NPU 上 triton kernel 路径；无 NPU 退化为 CPU
        参考实现时返回 fp32）。
    """
    assert x.dim() == 4, f"x must be 4D [B,T,H,K], got shape {tuple(x.shape)}"
    assert _is_power_of_two(chunk_size), "chunk_size must be a power of 2"
    ...
    # 输入统一转 fp16 并搬上 NPU（不会改动调用方传入的张量；已是 fp16 则 .to 为 no-op）
    x = x.to(torch.float16).to("npu")
    # 参数张量保持 fp32: A_log/dt_bias 每 (head,通道) 一个标量,参与 fp32 vector 运算;
    # 目标 case 下 gk 量级可达 O(1e3),fp16 参数/输出会引入 ~5e-4 相对误差 → 超 1e-2 门槛。
    ...
    # 输出保持 fp32（精度契约; kernel store 按 o.dtype.element_ty cast,自动适配）

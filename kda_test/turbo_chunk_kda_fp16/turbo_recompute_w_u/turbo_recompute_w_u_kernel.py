...
fp16 改造(2026-09-09,对齐 Ascend C 口径):输入 k/v/beta/A/gk 与输出 w/u/kg 统一 fp16;
kernel 内 load 后 vector 段(beta 乘、exp2(gk) 折叠与 kg 差分)保持 fp32, 喂给 tl.dot 的
操作数 cast 回 fp16 (Akk_inv 来自 fp16 GM 直喂 dot 不转 fp32; dot acc 仍 fp32), store
前才 cast 到缓冲元素 dtype。
"""
...
def turbo_recompute_w_u_torch(
    k, v, beta, A, gk=None,
    chunk_size=_DEFAULT_BT,
):
    """torch 元算子版本: 与 ``turbo_recompute_w_u_ref`` 数学一致, 但按 chunk 批量化。
...
def turbo_recompute_w_u_triton(
    k, v, beta, A, gk=None,
    chunk_size=_DEFAULT_BT,
    num_warps=4,
):

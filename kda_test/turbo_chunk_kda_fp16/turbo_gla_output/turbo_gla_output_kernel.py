...
fp16 改造(2026-09-09,对齐 Ascend C 口径):输入/输出 fp16;q·exp2(g) 衰减乘在 fp32
...
def chunk_gla_fwd_kernel_o(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        b_g = tl.load(g + (bos * H + i_h) * K + i_t * BT * s_q_t + i_k * BK + q_offs,
                      mask=q_mask, other=0.0)
        b_qg = (b_q * tl.math.exp2(b_g.to(tl.float32))).to(tl.float16)  # 衰减乘 fp32 完成；dot 操作数 cast fp16

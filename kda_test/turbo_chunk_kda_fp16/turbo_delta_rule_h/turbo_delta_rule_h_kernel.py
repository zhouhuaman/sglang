...
def turbo_delta_rule_h_ref(
    ...
    B, T, H, K = k.shape
    V = u.shape[-1]
    assert K == V, f"turbo_delta_rule_h_ref requires K==V, got K={K}, V={V}"
    BT = int(chunk_size)
    NT = _cdiv(T, BT)
...
def turbo_delta_rule_h_torch(
    k, w, u, gk, initial_state, initial_state_indices,
    chunk_size=_BT,
):
    """torch_npu 元算子版本: 与 ``turbo_delta_rule_h_ref`` 数学一致, 在 NPU 上运行。
    ...
    参数与 ``turbo_delta_rule_h_ref`` 相同，所有张量须在 NPU 上。
    返回 (h, v_new)；initial_state 被 in-place 更新。
    """
    B, T, H, K = k.shape
    V = u.shape[-1]
    assert K == V, f"turbo_delta_rule_h_torch requires K==V, got K={K}, V={V}"
    BT = int(chunk_size)
    NT = _cdiv(T, BT)
...
def turbo_delta_rule_h_triton(
    ...
    纯 CPU / 无 NPU 环境下不可用（请用 ``turbo_delta_rule_h_ref``）。
    """
    B, T, Hg, K = k.shape
    V = u.shape[-1]
    H = u.shape[-2]
    assert K == V, f"turbo_delta_rule_h_triton requires K==V, got K={K}, V={V}"

...
@triton.jit
def _token_parallel_kernel_new(
    q,
    k,
    g,
    beta,
    ...
    NT: tl.constexpr = T // BT
    WORK: tl.constexpr = H * NT
    PER_CORE: tl.constexpr = WORK // tl.num_programs(0)
    ...
    # For Aqk: keep only four diagonal 16x16 blocks,
    # lower triangular inside each block.
    br = (o_r // BC).to(tl.float32)
    bc = (o_c // BC).to(tl.float32)

    ri = (o_r % BC).to(tl.float32)
    ci = (o_c % BC).to(tl.float32)

    same_subchunk = br[:, None] == bc[None, :]
    causal = ri[:, None] >= ci[None, :]

    aqk_keep = same_subchunk & causal
    ...
    strict = (
        (o_r % BC).to(tl.float32)[:, None]
        >
        o_cc.to(tl.float32)[None, :]
    )  # j < i inside local 16x16 block

    for work_id in tl.range(begin, end, num_stages=2):

        # head-major flattening
        h = work_id // NT
        c = work_id % NT
        ...
        # T=16384 is exactly divisible by BT=64.
        Q = tl.load(q_ptr).to(tl.float32)        # [64,128]
        Kx = tl.load(k_ptr).to(tl.float32)       # [64,128]
        G = tl.load(g_ptr).to(tl.float32)        # [64,128]
        BETA = tl.load(beta_ptr).to(tl.float32)  # [64]
        ...
        # Pull factors outside K reduction.
        Aqk_full *= scale
        Akk_full *= BETA[:, None]
...
def token_parallel_triton(
    ...
    _token_parallel_kernel_new[grid](
        q, k, gk, beta,
        Aqk, Akk,
        scale,
        B=B,

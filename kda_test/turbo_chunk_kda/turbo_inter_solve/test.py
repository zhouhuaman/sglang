...
"""K3 turbo_inter_solve 单算子测试脚本（本目录自包含）。

你只需读懂/改动的文件是本目录的 `turbo_inter_solve_kernel.py`（含 torch 参考 + triton
...
import turbo_inter_solve_kernel as K  # noqa: E402
...
def _run_torch(i):
    return K.turbo_inter_solve_torch(i["q"], i["k"], i["g"], i["beta"], i["Akkd"],
                               i["scale"], chunk_size=BT, sub_chunk_size=BC)


def _run_triton(i):
    return K.turbo_inter_solve_triton(i["q"], i["k"], i["g"], i["beta"], i["Akkd"],
                                i["scale"], chunk_size=BT, sub_chunk_size=BC)
...
def main(argv=None):
    p = argparse.ArgumentParser(description="K3 turbo_inter_solve 测试/复现脚本")

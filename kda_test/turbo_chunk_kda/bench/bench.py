...
"""KDA 6 算子统一 bench（turbo_chunk_kda/bench 版）: 正确性（模式 A）+ msprof 分段采集（模式 B）。

本目录 kernel 从 ../turbo_gate_chunk_cumsum/ ~ ../turbo_gla_output/ 的独立实现导入
（与各算子目录 *_kernel.py 同源），替代 design/<op>/src 版，其余流程不变。
...
INTERVIEW = os.path.dirname(HERE)
# 把 turbo_chunk_kda 的 6 个算子目录加进 sys.path，以便 import 各 kernel 模块。
# 注意本目录与 design/<op>/src 的模块/函数命名不同:
#   K1: turbo_gate_chunk_cumsum_kernel.py（turbo_gate_chunk_cumsum_triton / _torch / ref）
#   K6: driver 名为 turbo_gla_output_triton（design 里叫 turbo_gla_output_kernel）
for d in ("turbo_gate_chunk_cumsum", "turbo_token_parallel", "turbo_inter_solve",
    ...
    sys.path.insert(0, os.path.join(INTERVIEW, d))
...
# 6 个算子的 driver 函数
from turbo_gate_chunk_cumsum_kernel import (  # noqa: E402
    RCP_LN2,
...
from turbo_inter_solve_kernel import (  # noqa: E402
    turbo_inter_solve_torch as k3_torch,
    turbo_inter_solve_triton as k3_triton,
)
from turbo_recompute_w_u_kernel import (  # noqa: E402
    turbo_recompute_w_u_torch as k4_torch,
    turbo_recompute_w_u_triton as k4_triton,
)
from turbo_delta_rule_h_kernel import (  # noqa: E402
    turbo_delta_rule_h_torch as k5_torch,
    turbo_delta_rule_h_triton as k5_triton,
)
from turbo_gla_output_kernel import (  # noqa: E402
    turbo_gla_output_torch as k6_torch,
    turbo_gla_output_triton as k6_triton,
)
...
KERNEL_META = [
    {"id": "K1", "name": "turbo_gate_chunk_cumsum",  "triton_op": "_gate_cumsum_kernel"},
    {"id": "K2", "name": "turbo_token_parallel",     "triton_op": "_token_parallel_kernel"},
    {"id": "K3", "name": "turbo_inter_solve",        "triton_op": "_inter_solve_kernel"},
    {"id": "K4", "name": "turbo_recompute_w_u",     "triton_op": "_recompute_w_u_kernel"},
    {"id": "K5", "name": "turbo_delta_rule_h",      "triton_op": "_delta_rule_h_kernel"},
    {"id": "K6", "name": "turbo_gla_output",        "triton_op": "chunk_gla_fwd_kernel_o"},
]
...
def _kernel_supports(kernel_id, B, T, H, K, V):
    ...
    已知约束:
      ...
      * K5 (delta_rule_h): 要求 K==V 且 K≤256 (K=128 已支持; store 统一走
        2D 手动指针 _store_h_full，K=64/非 64 一致)
